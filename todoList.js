var unpack = require("unpack-element")
    ...
    , render = writer(create, update, destroy)
...
function TodoListComponent(state, parent) {
    ...
    render(state.summaries, component)

    insert.prepend(parent, component.root)

    return queue.stream
}

function create(state, component) {
    console.log("create", state, component)
    var todo = TodoComponent(state, component)
}
...
function interactions(component) {
...
// function TodoListComponent(summaries, parent) {
//     var component = unpack(Element(todoListHtml))
//         , queue = component.queue = ReadStream()
